...
class Autoencoder:
    """
    Presenting the Autoencoder architecture
    ...
    def buildEncoder(self):
        ...
        encoder_bottleneck = self.addToLS(conv_layers)
    ...
    def addConvTransposeLayer(self, layerIndex, x):
        """Creation of transposed convolutional layer using train function"""
        ...
        convTransposeLayer = Conv2DTranspose(
            ...
            strides=self.convStrides[layerIndex], padding="same",
    ...
    def addToLS(self, x):
        """Compression of layers and pass through to latent space (dense layer)"""
        self.returnShape = K.int_shape(x)[1:]
        x = Flatten()(x)
        x = Dense(self.latentSpace, name="encoderOutput")(x)
        return x
    ...
    def compiler(self, learning_rate):
        ...
        loss_function = MeanSquaredError(name="MSE")
